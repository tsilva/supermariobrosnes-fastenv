...
- Stable Retro oracle/playback tooling targets `stable-retro-turbo==1.0.0.post23` and constructs `RetroVecEnv` with the new flat keyword names: `frame_maxpool`, `reset_noops`, `action_sticky_prob`, `info_filter`, `obs_copy`, and `done_on`. Runtime fired terminal rules are still read from `info["done_on_info"]`.
